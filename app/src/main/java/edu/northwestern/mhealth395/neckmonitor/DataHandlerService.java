...
public class DataHandlerService extends Service {
    ...
    public static long START_MINISEC = 0;
    public static String START_DATE = "";
    private static final String T_NECKLACE = "necklace";
    public boolean twoweek = true;
...
//                            values);
            ...
            if (isCsvStreaming) {
                Log.v(TAG, "posting data from store!!!!!!!!!!!!!");
                postToCSV(event);
            }
            return null;
        }
    ...
    public void postToCSV(NecklaceEvent event) {
        {
            ...
            String filename = folder.toString() + "/" + studyId + " " + START_DATE + ".csv";
            long curminisec = System.currentTimeMillis();
            File file = new File(filename);
// If file does not exists, then create it.
            boolean fpExists = true;
            if (curminisec - START_MINISEC >= 604800000) {//if one week passed, create a new .csv file
                try {
                    Calendar cal = Calendar.getInstance();
                    SimpleDateFormat df = new SimpleDateFormat("dd-MMM-yyyy hh:mm:ss aa");
                    START_DATE = df.format(cal.getTime());
                    filename = folder.toString() + "/" + studyId + " " + START_DATE + ".csv";
